...
public class LogicImpl implements Logic{
    ...
    private Set<Position> shown = new HashSet<>();
    ...
    public Optional<Integer> temporary(Position p) {
       return Optional.of(p).filter(pp->this.selected.contains(p)).map(this.values::get);
    }

    @Override
    public boolean isOver() {
        List<Integer> remaining = values.keySet().stream().filter(p-> !this.shown.contains(p)).map(this.values::get).toList();
